...
| [notification_domain_design.md](./notification_domain_design.md) | 通知域：P0+P1 **已落地**（`SendDirect()`/Notifier 缝、`sys_notification_deliveries` 投递台账、三处裸发送点迁完、三端台账页、邮件文案 `pkg/mailtext`、SSE 事件类型注册表 `pkg/sseevent`），P2 **已落地六块（2026-09-20）**：站内信注册为 INTERNAL 渠道 + 定向发送改走缝 + 台账 `related_id`（全员广播刻意不入台账，理由与遗留决策点在 §4 P2）、收件行租户打标跟着受众走 + 修掉收件箱一处越权读、异步投递（入队前同步预检 + asynq 派发 + 台账 `request_id`/`attempts`）、台账 `SENDING` 超时清扫（系统级常驻 cron）、台账结论回写失败按出口上抛（异步交给 asynq 重投、同步不回错给业务调用方）、收件箱写侧由服务端钉定收件人归属（与读侧同形，撤销消息缺对象级授权一处已知未修在 §7）；**C 已落地（2026-09-20）**：路由从两张 Go 静态表整体搬进 `sys_notification_rules`（空表播种、不缓存、删行即决定）、WEBHOOK 渠道（`webhook_url`/`webhook_secret` 两列 + dial 时按解析后 IP 的 SSRF 默认拦内网 + HMAC 时间戳签名 + 302 不跟随）、三端规则管理页、按规则行的测试投递 RPC；**C7 已落地（2026-09-20）**：全新安装播种链路在干净库上实测（4 行默认规则 / 菜单 68-72-73 / `sys_apis` 空表自动全量同步），并补上通知域的服务层平台超管判定——实测到平台渠道配置对租户管理员可读可写，而"挡住"另外两口的只是模块映射少登记两个服务的偶然 fail-closed，故 `requirePlatformAdmin` 挂满 14 个方法、映射表同步补齐；**欠账 1 已落地（2026-09-21）**：台账 `channel_id` 在失败路径与异步重试中途也照落（`Sender` 契约改为"err 非 nil 时回执仍带已确定的事实"，连带让异步重试从第二次起显式钉住第一次那条渠道配置）；**欠账 2 已定位未修（2026-09-21）**：租户侧边栏整箱被清空的成因 = `TenantRepo` 读不到 ent 的 `plan_id` 边外键（跨域，修法见该节，另在 `plan_billing.md` §10 留了指针）；**M 已落地（2026-09-21）**：通知的三个管理页从「系统管理」提成一级菜单 `/notification`（种子新增根 74 + 三端路由/页面目录/词条同步搬；老库须走各端「菜单同步」才多出这一行，陈旧行要手工清），顺带修掉一处让「新建菜单 / 新建 API / 菜单同步」必撞主键的缺陷——显式 ID 播种不推进 PG 的 identity 序列，现由 `MenuService`/`ApiService` 启动期与 `SyncApis` 之后各对齐一次（只前进不回头）；**N 已落地（2026-09-21）**：WEBHOOK 出口按渠道行配**出站风格**（`webhook_sign_style` = CUSTOM/NONE/DINGTALK/FEISHU/WECOM，签名怎么算、放 query 还是 body 还是头）与**载荷模板**（`webhook_payload_template`，单趟 `{{name}}` 替换、值按 JSON 转义、`json.Valid` 闸门在拨号前、坏模板由入队前预检拒成 SKIPPED 且零次拨号），三家群机器人回 HTTP 200 里的失败按 body 的 `errcode`/`code` 判成败，三端渠道页加风格列与模板输入并处理 i18next/vue-i18n 吃掉双花括号的坑；渠道类型仍只有 WEBHOOK 一种（厂商差异是出站形状、不是出口，见 §6 决策点 2 的 A/B 之争）；P3（用户偏好/退订/静音时段与消息模板管理）与 SMS 出口仍为设计提案（**WEBHOOK 的载荷模板已由 N 落地，不是 P3 那一档**）；含现状盘点、与 go-wind-im 的可抄性对照、运行期实测发现、移植记录与代码生成坑 | 动通知相关代码前先读；推进 P2/P3、改台账或路由规则语义前回来更新状态标记 |
