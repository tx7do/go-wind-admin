...
| [notification_domain_design.md](./notification_domain_design.md) | 通知域：P0+P1 **已落地**（`SendDirect()`/Notifier 缝、`sys_notification_deliveries` 投递台账、三处裸发送点迁完、三端台账页、邮件文案 `pkg/mailtext`、SSE 事件类型注册表 `pkg/sseevent`），P2 **已落地六块（2026-09-20）**：站内信注册为 INTERNAL 渠道 + 定向发送改走缝 + 台账 `related_id`（全员广播刻意不入台账，理由与遗留决策点在 §4 P2）、收件行租户打标跟着受众走 + 修掉收件箱一处越权读、异步投递（入队前同步预检 + asynq 派发 + 台账 `request_id`/`attempts`）、台账 `SENDING` 超时清扫（系统级常驻 cron）、台账结论回写失败按出口上抛（异步交给 asynq 重投、同步不回错给业务调用方）、收件箱写侧由服务端钉定收件人归属（与读侧同形，撤销消息缺对象级授权一处已知未修在 §7）；**C 已落地（2026-09-20）**：路由从两张 Go 静态表整体搬进 `sys_notification_rules`（空表播种、不缓存、删行即决定）、WEBHOOK 渠道（`webhook_url`/`webhook_secret` 两列 + dial 时按解析后 IP 的 SSRF 默认拦内网 + HMAC 时间戳签名 + 302 不跟随）、三端规则管理页、按规则行的测试投递 RPC；P3（偏好/模板）与 SMS 出口仍为设计提案；含现状盘点、与 go-wind-im 的可抄性对照、运行期实测发现、移植记录与代码生成坑 | 动通知相关代码前先读；推进 P2/P3、改台账或路由规则语义前回来更新状态标记 |
