...
const (
	...
	// `n^4 + 15 + rand(0..29)*(n+1)` 秒。n 是 msg.Retried，首次失败时还是 0（asynq 只在 Retry 里
	// 自增计数，processor.go 用自增前的值算延迟），所以三次退避落在 15~44 / 16~74 / 31~118 秒
	// （第 4 次失败时 Retried 已到 MaxRetry，直接归档、不再退避）——退避带随机数，预算是个区间：
	// **退避合计 62~236s，加满 4 次 30s 的处理时间后最坏 ≈ 356s ≈ 5.9 分钟**。实测两抽
	// ~101s（P2-3 快失败）与 ~179s（P2-5 回写失败）都是"尝试本身很快"的抽样，落在 62~236 里，
	// 不能当上界用。阈值取缺省 15 分钟 = 最坏预算的 2.5 倍，余量是给队列积压的。
	deliverySweepDefaultStaleAfter = 15 * time.Minute

	// deliverySweepMinStaleAfter 环境变量允许调到的下限。
	// 低于上面 5.9 分钟的最坏预算就会把"还在重试"的投递扫成 FAILED —— 而定案的行会被
	// AsyncNotificationDispatch 的幂等门挡掉，等于清扫亲手取消了一次还能救的投递。宁慢不误。
	// 10 分钟这个数是 P2-5 实测之后从 5 分钟抬上来的：5 分钟(300s) 仍落在最坏预算之内。
	// 首稿的"退避 2s/17s/82s ≈ 221s"是 asynq 旧版公式（无随机项、且漏了 n 从 0 起算），
	// 中间一版按 n=1,2,3 算出的"16~74/31~118/96~210 ⇒ 最坏 504s"同样不对。
